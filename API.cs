    ...
    public static class API
    {
        ...
        public static void SendKillWebhook(Player target, Player attacker, bool cuffed, bool suicide, DamageHandler handler)
        {
            ...
            if (!cuffed && !suicide)
                ...
                message = Plugin.instance.Config.webhookSuicideText.Replace("{Time}", DateTime.Now.ToString()).Replace("{target}", target.Nickname).Replace("{targetID}", target.RawUserId).Replace("{targetRole}", target.Role.ToString()).Replace("{damageType}", handler.Type.ToString()).Replace("{roleID}", Plugin.instance.Config.staffRoleID);
