    ...
    public class EventsHandler
    {
        ...
        public void OnHurt(HurtingEventArgs ev)
        {
            ...
            if (Plugin.instance.Config.logViaDiscordWebhook && Plugin.instance.Config.logHurting)
                API.QueueMessage(ev.Target, ev.Attacker, API.MessageType.Hurting, ev.Handler);
        }
        ...
        public void OnDying(DyingEventArgs ev)
        {
            if ((ev.Killer == null || ev.Killer == ev.Target) && Plugin.instance.Config.logSuicides && !Plugin.instance.Config.deathsNotToCountAsSuicide.Contains(ev.Handler.Type.ToString()))
            {
                ...
                if (Plugin.instance.Config.logViaDiscordWebhook)
                    API.QueueMessage(ev.Target, null, API.MessageType.Suicide, ev.Handler);
            ...
            if (ev.Target.IsCuffed && Plugin.instance.Config.logCuffedKills)
            {
                ...
                if(Plugin.instance.Config.logViaDiscordWebhook)
                    API.QueueMessage(ev.Target, ev.Killer, API.MessageType.Cuffed, ev.Handler);
            ...
            if ((ev.Killer.Role.Side != ev.Target.Role.Side) || ev.Handler.Type == DamageType.Recontainment)
                return;
            ...
            if(Plugin.instance.Config.logViaDiscordWebhook)
                API.QueueMessage(ev.Target, ev.Killer, API.MessageType.Normal, ev.Handler);
        }
    }
}
